...
2. 데이터를 변경하는 모든 서비스로직에 트랜잭션 적용하기
3. 미디어관련 엔티티가 데이터베이스에서 삭제 되면 서버 폴더내에 미디어 파일 물리적으로 삭제하기