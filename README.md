...
34. 엔티티 인스턴스를 가져오기 위해서 Searcher 클래스 내부의 메서드를 사용하였다. 이 때 가져오게 되는 엔티티는 상황에 따라서 조인이 필요한 경우와 필요하지 않은 경우가 존재한다. 기존에는 각 상황에 따라서 조인이 필요한 경우가 하드코딩으로 되어 있기 때문에 유연성이 부족하였다. 만약 User 엔티티를 가져온다 했을 때 단순히 
    Profile과 Auth만 필요한 경우와 Admin까지 필요한 경우가 있다 했을 때 기존 방식이라면 메서드가 2개 필요했을 것이다. 혹은 엔티티를 찾는 방법 또한 문제가 있었는데 만약 Account 엔티티를 찾는다 가정했을 때 Account의 아이디로 찾는 경우와 사용자 아이디로 찾는 경우와 찾으려는 계좌가 주 사용 계좌로 찾는 경우 3가지가 
    존재 하였다. 이를 위한 메서드가 3개나 필요한 셈이다. 또한 엔티티를 한개만 필요한지 혹은 여러개가 필요한지에 대한 여부또한 2개의 메서드가 필요하다. 단순히 엔티티를 찾는 코드가 중복이 되는것이 불편하게 생각이 되어 하나의 메서드에 옵션을 넣어서 엔티티를 찾을 수 있게 하였다. 들어가는 옵션은 where 구문 조건, where 구문의 
    사용될 값, 찾으려는 엔티티 단수 복수 여부, 조인할 엔티티 클래스 배열로 4가지가 존재한다. 엔티티 조인에 관해서 설명을 더 하자면 만약 Product 엔티티를 통해서 ClientUser 엔티티와 조인을 한다 가정할 때 Product -> Review -> ClientUser 이런식으로 조인을 한다면 Product에서 ClientUser와 
    접근이 가능해진다. 하지만 해당 솔루션은 이런 방식으로 depth가 2이상으로 조인이 불가능 하도록 되어있다. Product 엔티티를 찾는 메서드를 호출하게 될 때 엔티티 배열의 길이만큼 반복문을 돌려서 해당 배열의 아이템이 Product와 형제 관계에 있는 연관관계 필드만 조인이 가능하도록 되어있다. ClientUser는 Product와 
    형제 관계에 있는 연관관계 필드가 아니기 때문에 이런식으로 조인이 불가능하며 만일 조인할 엔티티 클래스 배열에 형제 관계에 있지 않은 엔티티 클래스를 넣을 경우 "None Relation With"으로 시작하는 에러를 로깅하게 된다. 한마디로 "관계가 존재하지 않은 클래스와 조인을 시도했다" 라는 뜻으로 이해 할 수 있을 것이다. 따라서 
    Product에서 ClientUser에 접근하려는 시도가 필요할 경우 우선 ProductSearcher에 있는 findEntity 메서드에 Review와 조인을 우선 시도한다. 그럴 경우 Product 엔티티에 Review 엔티티의 인스턴스가 존재할 것이다. 여기서 review 아이디를 추출하여 ReviewSearcher에 있는 
    findEntity 메서드에 where 조건과 where 구문에 사용될 값으로 review id를 설정한 후 조인 테이블 배열에 ClientUser를 넣어주면 된다. 아쉽게도 depth가 2이상인 테이블과 조인이 필요할 경우 이런식으로 findEntity를 두번 호출해야 한다는 단점이 존재하지만 이런 단점이외에 옵션을 통해서 엔티티를 찾기 
    때문에 각 기능 별로 메서드를 하드 코딩을 할 필요가 없어진 점은 썩 괜찮다. // todo 14 완료
35. 웹, 앱등에서 표시할 데이터를 가져오는 API에는 전체 목록을 가져오는 API와 해당 목록의 세부 사항을 불러오는 API가 필요에 따라 존재할 것이다. 기존에는 전체 목록을 가져오는 API가 2개의 API로 분류되었는데 생성 날짜를 기준으로 최신순과 오래된 순으로 가져오는 API가 각각 있었다. 이 방식또한 34번 회고 처럼 하드 코딩으로 
    되어 있다. 만약 전체 Product를 가져오는 경우 가격순으로 가져오거나 카테고리 별로 가져온다 했을 때 API를 하드 코딩으로 필요한 옵션별로 여러 개 생성이 필요하게 될 것이다. 그래서 Searcher 클래스에 findAllRaws를 호출해서 가져올 수 있도록 하였다. 이 메서드의 옵션은 findEntity를 할 때 처럼 옵션이 고정 
    되어 있지 않고 각 도메인에 따라서 옵션이 다르게 설정된다. Product와 InquiryRequest를 비교해보면 Product는 가격순이나 상품 카테고리 혹은 검색한 이름이 포함되는 여부로 옵션이 존재한다. InquiryRequest는 해당 문의 옵션이나 답변이 달린 문의 요청인지에 따라서 옵션이 갈리게 된다. 이러한 이유로 각 도메인에 
    들어갈 옵션은 각각 하드 코딩으로 되어있다. 또한 데이터를 가져오게 될 때 이전 방식은 repository에서 찾은 엔티티를 그대로 응답으로 내보내었지만 그대로 내보낸 데이터의 형식이 클라이언트가 사용하기 살짝 불편한 방식으로 보내지기 때문에 데이터를 살짝 가공해서 내보낼 필요가 있었다. // todo 10, 15 완료
36. 34, 35번 회고에서 언급된 findEntity, findAllRaws 메서드들은 각 Searcher 클래스에서 동일하게 사용되어야 하는 부분이기 때문에 인터페이스를 활용하여 모든 Searcher 클래스에서 사용 해야만 하게 해두었다. 또한 SearchRepository 클래스 역시 조인이 필요한 클래스를 찾는 
    findOptionalEntity 메서드, 순수 엔티티를 찾는 findPureEntity 메서드, repository에서 찾은 데이터를 가공해서 내보내는 findAllRaws 메서드 역시 인터페이스로 묶어 두어 사용해야만 하게 해두었다. findDetailRaw 메서드는 필요한 도메인과 굳이 필요없는 도메인이 분류 되어 있어서 이 메서드는 
    따로 인터페이스로 묶지 않았다. 잘했지?
37. 이메일과 비밀번호를 클라이언트쪽에서 서버로 전달할 때 방식을 변경하였다. 기존에는 request body에 그대로 이메일과 비밀번호를 노출하였지만 약간의 보안성을 추가하기 위하여 이제는 Basic Auth를 통해 이메일과 비밀번호를 보내게 하였다. 이 방식을 사용하면 Base64로 인코딩 되어져 보내지기 때문에 기존에 날것으로 보내지던 
    이메일과 비밀번호를 약간 더 안전하게 보낼 수 있게 되었다.  
38. 잠시 swagger로 API 문서화를 보류 하고 플러터로 클라이언트 앱을 개발하는데 집중 해야 할 것 같다. 이 프로젝트에서는 더 이상 커밋이 일어나지 않게 할 것이고 프로젝트를 fork 하여 새로운 리파지토리를 만든 이후 그곳에서 작업이 발생하게 할 것이다. 이 프로젝트는 nestjs로 쇼핑몰 backend prototype을 개발하기 위한 
    프로젝트 였으므로 이 프로젝트를 fork 해서 실제 서비스 되기 위한 프로젝트로 다시 만들것이다. 
## TO DO
